        ...
        ?>
                ...
                <script<?php echo Adminer\nonce() ?> type="text/javascript">
                    ...
                    domReady(() => {
                        ...
                        formp1.insertAdjacentHTML('beforeend', '<label style="margin-left:3px;"><input type="checkbox"
                            name = "add_favorite" value = "1" /> <?php echo $this->lang("Add to Favorites") ?></label > ');
                    ...
                    let favorite_queries = "";
                        ...
                        for (let query_hash in reversedStorage) {
                            favorite_queries += `
            <div>
                <a title="<?php echo $this->lang("Use Favorite") ?>"
                    href="?username=<?php echo $_GET['username'] ?>&db=<?php echo $_GET['db'] ?>&sql=${storage[query_hash]['query']}"><img
                        style="padding:1px; width: 15px; "
                ...
                <a title="<?php echo $this->lang("Delete Favorite") ?>" class="remove_favorite"
                    href="?username=<?php echo $_GET['username'] ?>&db=<?php echo $_GET['db'] ?>&sql=&remove_favorite=${query_hash}"><img
                        style="padding:1px; width: 15px; "
                ...
                <code
                    style="display: inline-block; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:600px; vertical-align:middle;">`+ jush.highlight("sql", decodeURIComponent(storage[query_hash]['query'])) + `</code>
            </div>`;
                        }
            ...
            <div>
                <fieldset>
                    <legend><a href="#favorite-queries"><?php echo $this->lang("Favorites") ?></a></legend>
